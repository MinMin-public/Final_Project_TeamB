...
TRAJECTORY_BUILDER_2D.voxel_filter_size = 0.2 -- 0.1 --클수록 계산이 훨씬 빠르게 진행되어 금방 위치를 잡음
TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.max_length = 0.9
TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.min_num_points = 100--100
...
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 4.5--10. --큰 값을 가질수록 차는 움직여도 자기 위치는 그대로를 유지하려고 하는 특성을 가짐(4또는 5정도가 제일 좋은 성향을 보여줌)
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.rotation_weight = 20.5--20--30--40. --위의 경우와 비슷함, 작을수록 더욱 빨리 위치를 잡는 느낌을 보여줌
...
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 20--15 --속도가 빠를때 이 값이 크면 위치를 잘 잡지 못함(느릴때 너무 작은 값이면 위치를 잘 못잡음)
...
POSE_GRAPH.constraint_builder.global_localization_min_score = 0.62 --클 수록 위치를 빠르게 잡지 않음 0.6이 적당한 수준의 값으로 보임
...
POSE_GRAPH.global_sampling_ratio = 0.1--0.015
