...
TRAJECTORY_BUILDER_2D.voxel_filter_size = 0.4--0.2 -- 0.1 --클수록 계산이 훨씬 빠르게 진행되어 금방 위치를 잡음
TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.max_length = 0.9
TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.min_num_points = 150--100
TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.max_range = 20.--30
TRAJECTORY_BUILDER_2D.loop_closure_adaptive_voxel_filter.max_length = 0.9
TRAJECTORY_BUILDER_2D.loop_closure_adaptive_voxel_filter.min_num_points = 150--100
TRAJECTORY_BUILDER_2D.loop_closure_adaptive_voxel_filter.max_range = 20.--30.
...
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 2.--8.--4.5--10. --큰 값을 가질수록 차는 움직여도 자기 위치는 그대로를 유지하려고 하는 특성을 가짐(4또는 5정도가 제일 좋은 성향을 보여줌)
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.rotation_weight = 12.--22.--20.5--20--30--40. --위의 경우와 비슷함, 작을수록 더욱 빨리 위치를 잡는 느낌을 보여줌
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.ceres_solver_options.use_nonmonotonic_steps = false
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.ceres_solver_options.max_num_iterations = 20
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.ceres_solver_options.num_threads = 4
TRAJECTORY_BUILDER_2D.motion_filter.max_time_seconds = 0.3--0.3--0.2 값이 클 수록 더 데이터를 적게받아서 그런지 처리속도 증가
TRAJECTORY_BUILDER_2D.motion_filter.max_distance_meters = 0.2--0.2--0.1 역시 클 수록 데이터를 적게받아 처리속도 증가하는 것으로 보임
TRAJECTORY_BUILDER_2D.motion_filter.max_angle_radians = math.rad(0.5)
TRAJECTORY_BUILDER_2D.imu_gravity_time_constant = 10.
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 22--22--15--15 --속도가 빠를때 이 값이 크면 위치를 잘 잡지 못함(느릴때 너무 작은 값이면 위치를 잘 못잡음)
...
POSE_GRAPH.constraint_builder.global_localization_min_score = 0.61 --클 수록 위치를 빠르게 잡지 않음 0.6이 적당한 수준의 값으로 보임
...
POSE_GRAPH.global_sampling_ratio = 0.04--0.3--0.1--0.015
