...
class YOLO:
    ...
    def image_detect(self, box_data, person, cat):
        #print('image_detect')
        person = person
        cat = cat
        ...
        if boxes is not None:
            for i in range(len(boxes.bounding_boxes)):
                ...
                #print(width, height)
                #if (boxes.bounding_boxes[i].Class == object1 and person == True) and boxes.bounding_boxes[i].probability >= 0.35 and width*height > 37000:
                    #person = True
                    #print('Class : ', boxes.bounding_boxes[i].Class)
                    #return 'person'
                if (boxes.bounding_boxes[i].Class == object2 and cat == True) and boxes.bounding_boxes[i].probability >= 0.35 and width*height > 12500:
                    #cat = True
                    #print('Class : ', boxes.bounding_boxes[i].Class)
                    return 'cat'
                else:
